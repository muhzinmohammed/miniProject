...
jscalternate         bool                 NOT NULL DEFAULT true,
jscinterval          int2                 NOT NULL DEFAULT 0,
jscdst               bool                 NOT NULL DEFAULT false,
jscdststart          timestamptz          NULL,
jscdstend           timestamptz           NULL,
...
CREATE OR REPLACE FUNCTION pgagent.pga_next_schedule(int4, timestamptz, timestamptz, _bool, _bool, _bool, _bool, _bool, _bool, bool, timestamptz, int2, bool, timestamptz, timestamptz) RETURNS timestamptz AS '
DECLARE
    jscid           ALIAS FOR $1;
    ...
    jscalternate    ALIAS FOR $10;
    lastrun         ALIAS FOR $11;
    jscinterval     ALIAS FOR $12;
    jscdst          ALIAS FOR $13;
    jscdststart     ALIAS FOR $14;
    jscdstend       ALIAS FOR $15;
...
--new code
    ...
    IF jscalternate = TRUE THEN
        IF lastrun IS NOT NULL THEN
            nextrun := lastrun + INTERVAL ''2 Days'';
        ELSE
            nextrun := runafter;
        END IF;
    ELSIF jscinterval > 0 THEN
         IF lastrun IS NOT NULL THEN
            nextrun := lastrun + make_interval(days => jscinterval);
        ELSE
            nextrun := runafter;
        END IF;
    ELSE
--new code end
    ...
    IF jscdst = TRUE THEN
        IF nextrun >= jscdststart AND nextrun <= jscdstend THEN
            IF lastrun = NULL OR lastrun < jscdststart OR bingo = TRUE THEN
                nextrun := nextrun + INTERVAL ''1 hour'';
            END IF;
        ELSIF nextrun > jscdstend THEN
            IF lastrun IS NOT NULL AND lastrun <=jscdstend THEN
                nextrun := nextrun - INTERVAL ''1 hour'';
            END IF;
        END IF;
    END IF;
...
END;
' LANGUAGE 'plpgsql' VOLATILE;
COMMENT ON FUNCTION pgagent.pga_next_schedule(int4, timestamptz, timestamptz, _bool, _bool, _bool, _bool, _bool, _bool, bool, timestamptz, int2, bool, timestamptz, timestamptz) IS 'Calculates the next runtime for a given schedule';
    ...
    IF NEW.jobenabled THEN
        IF NEW.jobnextrun IS NULL THEN
             SELECT INTO NEW.jobnextrun
              ...
              WHERE s.jscenabled AND s.jscjobid=OLD.jobid;
        END IF;
    ELSE
        NEW.jobnextrun := NULL;
    END IF;
